...
//HypeCoin Walletd API Wrapper
//https://github.com/HazeDevelopment/hypecoin-api-php
//Forked from https://github.com/KittyCatTech/forknote-api-php
...
class HypeWalletd {
   ...
   public function getTransactionHashes( $firstBlockIndex=false, $firstblockHash=false, $blockCount, $paymentId=false, $addresses=false ) {
	...
	// }
	// @TODO Test
   public function getTransactions( $firstBlockIndex=null, $firstblockHash=null, $blockCount, $paymentId=null, $addresses=null ) {
      $args = array();
      $args["jsonrpc"] = "2.0";
      $args["id"] = "test";
   ...
   public function getAllTransactions( $firstBlockIndex, $blockCount, $addresses ) {
	$args = array();
...
//if( is_array($hype->save()) ) echo "saved\n"; else echo "not saved\n";

//print_r( $view_key = $hype->getViewKey() );

//print_r( $status = $hype->getStatus() );

//print_r( $addr = $hype->createAddress() );

//print_r( $addr = $hype->getAddresses() );

//print_r( $hype->getSpendKeys($addr["addresses"][0]) );

//print_r( $hype->getSpendKeys("") );

//print_r( $hype->deleteAddress("") );

//print_r( $hype->reset("") );

//print_r( $addr = $hype->createAddress(false, "") );

//print_r( $addr = $hype->createAddress("") );

//print_r( $addr = $hype->createAddress() );

//print_r( $hype->getSpendKeys($addr["address"]) );

//print_r( $hype->deleteAddress($addr["addresses"][0]) );

//print_r( $addr = $hype->createAddress("") );

//print_r( $hype->getBalance("") );
...
//print_r( $hype->getBlockHashes( $status["blockCount"] - 20, 20 ) );

//$starttime = time();
//$hype->getBlockHashes( 1, $status["blockCount"] );
//echo "to get all hash: ". (time()- $starttime) . " ms\n";

print_r( $hype->getTransactionHashes( $status["blockCount"] - 150, false, 150) );

print_r( $hype->getTransactions( $status["blockCount"] - 1500, false, 1500) );

print_r( $hype->getUnconfirmedTransactionHashes() );

//print_r( $hype->getTransaction( $transactionHash ));

echo $hype->makePaymentId() . PHP_EOL;
*/
//sleep(20);
//}
